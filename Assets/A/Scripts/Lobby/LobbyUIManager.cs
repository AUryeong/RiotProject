...
namespace Lobby
{
    public class LobbyUIManager : MonoBehaviour
    {
        ...
        public void ActiveSetting()
        {
            ...
            bgmButton.image.rectTransform.DOScale(Vector3.one, UI_SCALE_DURATION).SetEase(Ease.OutBack).SetDelay(UI_MOVE_DURATION);

            settingButton.image.rectTransform.localScale = Vector3.zero;
            settingButton.image.rectTransform.DOScale(Vector3.one, UI_SCALE_DURATION).SetEase(Ease.OutBack).SetDelay(UI_MOVE_DURATION + UI_SCALE_DURATION / 2);

            shopButton.image.rectTransform.localScale = Vector3.zero;
            shopButton.image.rectTransform.DOScale(Vector3.one, UI_SCALE_DURATION).SetEase(Ease.OutBack).SetDelay(UI_MOVE_DURATION + UI_SCALE_DURATION);
        ...
        public void DeActiveSetting()
        {
            ...
            background.color = background.color.GetFade(0.3f);
            background.DOFade(0, UI_MOVE_DURATION).SetDelay(UI_SCALE_DURATION * 2).OnComplete(() => gameObject.SetActive(false));

            bgmButton.image.rectTransform.localScale = Vector3.one;
            bgmButton.image.rectTransform.DOScale(Vector3.zero, UI_SCALE_DURATION).SetEase(Ease.InBack);

            settingButton.image.rectTransform.localScale = Vector3.one;
            settingButton.image.rectTransform.DOScale(Vector3.zero, UI_SCALE_DURATION).SetEase(Ease.InBack).SetDelay(UI_SCALE_DURATION / 2);

            shopButton.image.rectTransform.localScale = Vector3.one;
            shopButton.image.rectTransform.DOScale(Vector3.zero, UI_SCALE_DURATION).SetEase(Ease.InBack).SetDelay(UI_SCALE_DURATION);

            runeIcon.rectTransform.anchoredPosition = new Vector2(-100, runeIcon.rectTransform.anchoredPosition.y);
            runeIcon.rectTransform.DOAnchorPosX(100, UI_MOVE_DURATION);

            startButton.image.rectTransform.anchoredPosition = Vector2.zero;
            startButton.image.rectTransform.DOAnchorPosY(-1200, UI_MOVE_DURATION).SetDelay(UI_SCALE_DURATION * 2);
        }
    }
}